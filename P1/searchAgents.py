...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # returning the corners we have visited and the starting position
        corners_state = []
        for corner in self.corners:
            corners_state.append(
                (corner, True)) if self.startingPosition == corner else corners_state.append((corner, False))
        corners_state = tuple(corners_state)
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        return not (False in [i[1] for i in state[1]])
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                for i in range(len(corners_state)):
                    if corners_state[i][0] == neighbor:
                        corners_state[i] = [j for j in corners_state[i]]
                        corners_state[i][1] = True
                        corners_state[i] = tuple(corners_state[i])
                corners_state = tuple(corners_state)
                successors.append(((neighbor, corners_state), action, 1))
        self._expanded += 1  # DO NOT CHANGE
        return successors
...
def cornersHeuristic(state, problem):
    ...
    for corner in state[1]:
        if corner[1] is False:
           manhattan_distances.append(calculate_manhattan(current_state, corner[0]))
    return max(manhattan_distances)


def calculate_manhattan(x, y):
...
def foodHeuristic(state, problem):
    ...
    costs = []
    if len(food_positions)==0:                                                  #Here we are using the mazeDistance for finding the distance to food
        return 0                                                         #and we are returning the max cost (Notation:It is taking 20 sec but)
    for i in food_positions:                                        #it expands only 4100 and something nodes).We can do the same with
        costs.append(mazeDistance(pos,i,problem.startingGameState))   #the cornersheuristis using manhattanheuristic but for food and corners
    max_cost = float("-inf")
    for j in costs:
        if max_cost < j :
            max_cost = j
